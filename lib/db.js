...
function config() {
  ...
  if (!tableName || !partitionKey || !sortKey || !process.env.AWS_REGION) throw new Error('DynamoDB is not configured.');
  return { tableName, partitionKey, sortKey };
}

function getDb() {
  if (!client) client = DynamoDBDocumentClient.from(new DynamoDBClient({ region: process.env.AWS_REGION }), { marshallOptions: { removeUndefinedValues: true } });
  return client;
}
